...
| BorderRadiusTransformer | Splits two-value `border-*-radius` pairs, emits four properties | `style_mapper_convert.go:204-211` - handles single values | Two-value pair splitting for `10px / 20px` syntax |
| ShapeOutsideTransformer | Accepts `polygon()` coordinates, validates percent pairs | `style_mapper_convert.go:214-216` - passes raw value to `yj.border_path` | Polygon coordinate parsing and validation |
...
| ex to em conversion | Converts ex using `com.amazon.yjhtmlmapper.i.c` | ex unit passed through (`SymUnitEx`) | Should convert using 0.5em ratio |

### Not Implemented

| Normalization | KP3 Purpose | Priority | Notes |
|---------------|-------------|----------|-------|
| text-decoration normalization | Removes "remove underline" for tags that control decoration | Medium | Should skip for `<u>`, `<a>`, `<ins>`, `<del>`, `<s>`, `<strike>`, `<br>` |
...
7. **text-decoration normalization**
