...
func fitPDFImageInBox(doc pdfDocumentSpec, img *fb2.BookImage, x, y, maxWidth, maxHeight float64) (pdfRect, bool) {
	...
	width, height, ok := fitPDFImageDimensions(width, height, maxWidth, maxHeight, false)
	if !ok {
...
func fitPDFBlockImageSize(
	doc pdfDocumentSpec,
	img *fb2.BookImage,
	maxWidth, maxHeight, widthReference float64,
	forceContentWidth bool,
	...
	if widthReference <= 0 {
		widthReference = maxWidth
	}

	width := maxWidth
	if !forceContentWidth {
		width = widthReference * min(float64(widthPx)/pdfKP3ContentWidthPx, 1)
...
func pdfBlockImageReferenceWidth(
	block pdfTextBlock,
	style pdfBlockResolvedStyle,
	availableWidth, rootlessContentWidth float64,
	img *fb2.BookImage,
	forceContentWidth bool,
) float64 {
	...
	return blockContentWidth(widthLimit, style)
}
