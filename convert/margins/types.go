...
const (
	...
	// This is used for stanzas where spacing between verses comes entirely from
...
// Each node corresponds to a rendered element and holds margin information
// that will be modified during the collapsing phases.
type ContentNode struct {
	// Index is the index into the originating element list.
	// A value of -1 indicates a virtual container node (no corresponding element).
	Index int

	// EID is an opaque element ID (for debugging/tracing).
	EID int

	// ContentType is a description of the content type (e.g., "text", "image").
	ContentType string

	// Style is the current style name (may be updated after collapsing).
	Style string

	// MarginTop is the margin-top value.
	// nil means no margin (will be removed from style).
	// 0.0 means explicit zero margin.
	MarginTop *float64

	// MarginBottom is the margin-bottom value.
	// nil means no margin (will be removed from style).
	MarginBottom *float64

	// HasBreakAfterAvoid is true if the element has page-break-after: avoid.
	...
	// in the output (a content entry with child elements).
	//
	// For such containers, margins stay on the wrapper itself and first/last-child
	// margins collapse into the wrapper (not by transferring the wrapper's margins
	// down to children). Purely virtual containers (no wrapper entry) still use
	// transfer semantics.
	HasWrapper bool
}
...
// It's built from a flat element list using container IDs.
type ContentTree struct {
	// Root is the virtual root node representing the top-level container.
	Root *ContentNode
	...
	// WrapperMap tracks which wrapper entry (by index) corresponds
...
func NewContentTree(tracer Tracer) *ContentTree {
	return &ContentTree{
		Root: &ContentNode{
			Index:         -1,
